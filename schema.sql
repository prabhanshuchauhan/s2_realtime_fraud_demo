...
DELIMITER //
CREATE OR REPLACE PROCEDURE fraud_proc_backup(
    batch QUERY(user_id INT, timestamp TEXT, lat DOUBLE, lon DOUBLE, amount DOUBLE, atm_id VARCHAR(64))
)
AS
BEGIN
    INSERT INTO transactions_backup(user_id, ts, lat, lon, amount, atm_id)
    SELECT
        user_id,
...
AS LOAD DATA KAFKA 'ec2-34-232-153-191.compute-1.amazonaws.com:9092/atm-users'
BATCH_INTERVAL 1000
...
AS LOAD DATA KAFKA 'ec2-34-232-153-191.compute-1.amazonaws.com:9092/atm-transactions'
BATCH_INTERVAL 1000
DISABLE OFFSETS METADATA GC
